// Course-generic content validator (generalizes validate-vt-content.mjs):
// counts, schema shape, figure files, SVG well-formedness, narration length.
//   node scripts/validate-content.mjs [courseId ...]     (default: all 8 courses)
//   node scripts/validate-content.mjs --module ut1-2     (validate one module only)
// Exit 1 if any PROBLEMS; warnings don't fail.
...
const onlyModule = modIdx > -1 ? args[modIdx + 1] : null;
const courseArgs = args.filter((a, i) => a !== "--module" && (modIdx === -1 || i !== modIdx + 1));
const ALL = ["ut", "rt", "mt", "pt", "et", "vt", "basic", "rad"];
const courses = courseArgs.length ? courseArgs : ALL;

let exitBad = false;

for (const courseId of courses) {
  const contentDir = join(webRoot, "src", "data", "content", courseId);
  const figuresDir = join(webRoot, "public", "content", courseId, "figures");
  ...
  const problems = [];
  const warn = [];
  let lessonsOk = 0, lessonsTotal = 0, questionsTotal = 0, figuresTotal = 0, words = 0;
  ...
  for (const level of curriculum.levels) {
    for (const mod of level.modules) {
      ...
      for (const lesson of mod.lessons) {
        ...
        if (courseId !== "vt" && (!c.imagePrompt || typeof c.imagePrompt !== "string" || c.imagePrompt.length < 30))
          problems.push(`${lesson.id}: imagePrompt missing/too short`);

        const wantsInteractive = lesson.media.some((m) => m.type === "interactive");
        const wantsSim = lesson.media.some((m) => m.type === "simulation");
        if (wantsInteractive && !c.interactive) problems.push(`${lesson.id}: interactive required but null`);
        if (courseId !== "vt" && wantsSim && !c.interactive && !c.simulator)
          problems.push(`${lesson.id}: simulation media requires an interactive (or simulator)`);
        if (c.interactive) {
          const t = c.interactive.type;
          if (!["scenario", "hotspot", "sort", "calculator"].includes(t)) problems.push(`${lesson.id}: bad interactive type ${t}`);
          if (t === "calculator" && courseId !== "vt") problems.push(`${lesson.id}: calculator interactive is VT-only`);
          if (t === "hotspot") {
        ...
        if (c.simulator && courseId !== "vt") problems.push(`${lesson.id}: simulator field is VT-only`);
  ...
  if (!onlyModule && existsSync(contentDir)) {
    const orphans = readdirSync(contentDir).filter((f) => f.endsWith(".json") && f !== "videos.json")
      .filter((f) => {
  ...
  console.log(`lessons present: ${lessonsOk}/${lessonsTotal} · questions: ${questionsTotal} · figures: ${figuresTotal} · narration words: ${words.toLocaleString()}`);
